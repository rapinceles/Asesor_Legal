...
try:
    ...
    print("⚠️  Base de datos no disponible - usando modo simplificado")
    DB_AVAILABLE = False
...
try:
    ...
    print("⚠️  Scrapers no disponibles - usando datos simulados")
...
try:
    ...
    print("⚠️  Motor de análisis AI no disponible - usando respuestas predefinidas")
    AI_ENGINE_AVAILABLE = False

# Crear las tablas en la base de datos (solo si DB está disponible)
if DB_AVAILABLE:
...
def get_db():
    if DB_AVAILABLE:
        ...
        # Retorna None si no hay DB disponible
        yield None
...
async def analisis_empresarial(
    ...
    try:
        ...
        if not DB_AVAILABLE:
            # Usar datos simulados si no hay DB
            return JSONResponse({
                "success": True,
                "respuesta": f"Análisis empresarial simulado para {company_name}. Sistema funcionando en modo simplificado.",
                "empresa_info": {
                    "nombre": company_name,
                    "estado": "Análisis completado (modo simulado)"
                }
            })
